...
export class ReviewOwnerOrAdminGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const isAllowed = await this.reviewsRepository.isOwnerOrAdmin(reviewId, userId)
    if (!isAllowed) {
      throw new ForbiddenException('No tienes permiso para realizar esta acción sobre esta reseña')
    }
